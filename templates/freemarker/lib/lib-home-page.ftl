...
<#-- Renders the html for the excellenz cluster section on the home page. -->
<#-- Works in conjunction with the homePageUtils.js file -->
<#macro exzellenzClusterHtml>
    <section id="home-exzellenz-cluster" class="home-sections">
        <h4>${i18n().excellence_clusters}</h4>
        <div id="exzellenz-Clusters">
        </div>
    </section>
</#macro>

<#-- builds the "exzellenz cluster" box on the home page -->
<#macro listExzellenzClusters>
<script>
var exzellenzClusterVar = [
<#if exzellenzClusters?has_content>
    <#list exzellenzClusters as resultRow>
        <#assign uri = resultRow["uri"] />
        <#assign label = resultRow["label"] />
        {"uri": "${uri?url}", "label": "${label}"}<#if (resultRow_has_next)>,</#if>
    </#list>
</#if>
];
...
<#-- Renders the html for the bua mitglieder section on the home page. -->
<#-- Works in conjunction with the homePageUtils.js file -->
<#macro buaMitGliederHtml>
    <section id="home-bua-mitglieder" class="home-sections">
        <h4>${i18n().bua_members}</h4>
        <div id="bua-mitglieder">
        </div>
    </section>
</#macro>


<#-- builds the "bua mitglieder" box on the home page -->
<#macro listBuaMitglieder>
<script>
var buaMitgliederVar = [
<#if buaMitglieder?has_content>
    <#list buaMitglieder as resultRow>
        <#assign uri = resultRow["uri"] />
        <#assign label = resultRow["label"] />
        {"uri": "${uri?url}", "label": "${label}"}<#if (resultRow_has_next)>,</#if>
    </#list>
...
<#-- Renders the html for the bua projects section on the home page. -->
<#-- Works in conjunction with the homePageUtils.js file -->
<#macro buaProjectHtml>
    <section id="home-bua-project" class="home-sections">
        <h4>${i18n().bua_projects}</h4>
        <div id="bua-projects">
        </div>
    </section>
</#macro>
...
<#-- builds the "bua projects" box on the home page -->
<#macro listBuaProjects>
<script>
var buaProjectsVar = [
<#if buaProjekte?has_content>
    <#list buaProjekte as resultRow>
        <#assign uri = resultRow["uri"] />
        <#assign label = resultRow["label"] />
        {"uri": "${uri?url}", "label": "${label}"}<#if (resultRow_has_next)>,</#if>
    </#list>
            ...
            <ul id="stats">
                ...
                <#list classGroups as group>
                    ...
                    <#if (group.individualCount > 0)>
                        ...
                        <#if !group.uri?contains("equipment") && !group.uri?contains("course") && !group.uri?contains("locations") && !group.uri?contains("activities")>
                            <li>
