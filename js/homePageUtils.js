...
function buildExzellenzClusters() {
        var deptNbr = exzellenzClusterVar.length;
        ...
        if ( deptNbr == 0 ) {
            html = "<ul style='list-style:none'><p><li style='padding-top:0.3em'>"
                   + i18nStrings.noClustersFound + "</li></p></ul>";
        }
        else if ( deptNbr > 6 ) {
        	//if there are more than 6 departments, we want to choose a random subset and display
        	//and also to make sure the same department is not repeated twice
        	...
        	while(indicesCount < 6) {
                ...
                if(!indexFound) {
                	//if this index hasn't already been employed then utilize it
                	 html += "<li><a href='" + urlsBase + "/individual?uri="
                     + exzellenzClusterVar[index].uri + "'>"
                     + exzellenzClusterVar[index].label + "</a></li>";
                	 //add this index to the set of already used indices
                	 indicesUsed[index] = true;
                	 //keep count
                	 indicesCount++;
            ...
            for ( var i=0;i<deptNbr;i++) {
                html += "<li><a href='" + urlsBase + "/individual?uri="
                        + exzellenzClusterVar[i].uri + "'>"
                        + exzellenzClusterVar[i].label + "</a></li>";
            }
        }
        if ( deptNbr > 0 ) {
            html += "</ul><ul style='list-style:none'>"
                    + "<li style='font-size:0.9em;text-align:right;padding: 6px 16px 0 0'><a href='"
                    + urlsBase
                    + "/organizations#http://vivo.berlin-university-alliance.de/ontology/core/v1/bua/Exzellenzcluster' alt='"
                    + i18nStrings.viewAllClusters + "'>"
                    + i18nStrings.viewAllString + "</a></li></ul>";
        }
        $('div#exzellenz-Clusters').html(html);
    }

    function buildBuaMitglieder() {
        var deptNbr = buaMitgliederVar.length;
        ...
        if ( deptNbr == 0 ) {
            html = "<ul style='list-style:none'><p><li style='padding-top:0.3em'>"
                   + i18nStrings.noMembersFound + "</li></p></ul>";
        }
        else if ( deptNbr > 6 ) {
        	//if there are more than 6 departments, we want to choose a random subset and display
        	//and also to make sure the same department is not repeated twice
        	...
        	while(indicesCount < 6) {
                ...
                if(!indexFound) {
                	//if this index hasn't already been employed then utilize it
                	 html += "<li><a href='" + urlsBase + "/individual?uri="
                     + buaMitgliederVar[index].uri + "'>"
                     + buaMitgliederVar[index].label + "</a></li>";
                	 //add this index to the set of already used indices
                	 indicesUsed[index] = true;
                	 //keep count
                	 indicesCount++;
            ...
            for ( var i=0;i<deptNbr;i++) {
                html += "<li><a href='" + urlsBase + "/individual?uri="
                        + buaMitgliederVar[i].uri + "'>"
                        + buaMitgliederVar[i].label + "</a></li>";
            }
        }
        if ( deptNbr > 0 ) {
            html += "</ul>";
        }
        $('div#bua-mitglieder').html(html);
    }

     function buildBuaProjects() {
        var deptNbr = buaProjectsVar.length;
        ...
        if ( deptNbr == 0 ) {
            html = "<ul style='list-style:none'><p><li style='padding-top:0.3em'>"
                   + i18nStrings.noProjectsFound + "</li></p></ul>";
        }
        else if ( deptNbr > 4 ) {
                //if there are more than 6 departments, we want to choose a random subset and display
                //and also to make sure the same department is not repeated twice
                ...
                if(!indexFound) {
                        //if this index hasn't already been employed then utilize it
                         html += "<li><a href='" + urlsBase + "/individual?uri="
                           + buaProjectsVar[index].uri + "'>"
                     + buaProjectsVar[index].label + "</a></li>";
                         //add this index to the set of already used indices
                         indicesUsed[index] = true;
                         //keep count
                         indicesCount++;
            ...
            for ( var i=0;i<deptNbr;i++) {
                html += "<li><a href='" + urlsBase + "/individual?uri="
                        + buaProjectsVar[i].uri + "'>"
                        + buaProjectsVar[i].label + "</a></li>";
            }
        }
         if ( deptNbr > 0 ) {
            html += "</ul><ul style='list-style:none'>"
                    + "<li style='font-size:0.9em;text-align:right;padding: 6px 16px 0 0'><a href='"
                    + urlsBase
                    + "/projects#http://vivoweb.org/ontology/core#Project' alt="
                    + i18nStrings.viewAllProjects + "'>"
                    + i18nStrings.viewAllString + "</a></li></ul>";
        }
        $('div#bua-projects').html(html);
    }
